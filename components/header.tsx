...
const NavigationItem = memo(({ href, label, isActive, onClick }: {
  href: string
  label: string
  isActive: boolean
  onClick?: () => void
...
export function Header() {
  ...
  return (
    <header key={`header-${forceUpdate}`} className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            ...
            {loading ? (
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 text-secondary-text transition-transform ${showUserMenu ? "rotate-180" : ""}`} />
                </button>
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-lg shadow-lg py-2 animate-in fade-in slide-in-from-top-2 z-50">
                    <div className="py-1">
                      <CustomLink
                        href="/configuracion"
